...
개발 전, 간단할줄만 알았던 로직들도 실제로 구현하는 과정이 생각보다 복잡했었다. 기획, 디자인부터 개발까지 스스로 진행하면서 정답이 없는 영역이라 생각되는 디자인부분이 상당히 막막한 느낌이였다. 

또한, 실제로 도메인 설정까지 끝내는 과정에서 설정하는 부분이 굉장히 다양해 신경쓸게 많았다. 

하지만, 이런 막막함 속에 UI/UX, 디자인, 배포 관련 지식도 쌓을 수 있는 좋은 기회였고, 칭찬과 격려속에 마무리 지을 수 있었다.